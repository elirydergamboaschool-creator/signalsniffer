...
static void subghz_cb(SubGhzWorker* w, void* ctx) {
    UNUSED(w);
    App* app = ctx;
    SubGhzProtocolDecoderBase* d = subghz_worker_get_decoder(app->sgw);
    if(!d) return;
    FuriString* s = furi_string_alloc();
    subghz_protocol_decoder_base_get_string(d, s);
    if(furi_string_size(s)) {
        furi_string_cat(app->log, s);
...
static void ir_cb(void* ctx, InfraredWorkerSignal* sig) {
    ...
    if(infrared_worker_signal_is_decoded(sig)) {
        ...
        snprintf(buf, sizeof(buf), "[IR] %s a=0x%lX c=0x%lX\n",
...
static void stop_all(App* app) {
    if(app->sgw) {
        subghz_worker_stop(app->sgw);
        subghz_worker_free(app->sgw);
        app->sgw = NULL;
...
static void menu_cb(void* ctx, uint32_t idx) {
    ...
    if(idx == MenuSubGHz) {
        ...
        app->sgw = subghz_worker_alloc();
        subghz_worker_set_overrun_callback(app->sgw, subghz_cb, app);
        furi_hal_subghz_reset();
        furi_hal_subghz_load_preset(FuriHalSubGhzPresetOok650Async);
        furi_hal_subghz_set_frequency_and_path(433920000);
        subghz_worker_start(app->sgw);
        ...
        if(!h) {
            ...
            uint32_t end = furi_get_tick() + furi_ms_to_ticks(3000);
            char line[128]; size_t pos = 0;
            while(furi_get_tick() < end) {
                uint8_t b = 0;
                if(furi_hal_serial_rx(h, &b, 1, 10)) {
                    if(b == '\n' || pos >= sizeof(line)-1) {
                        line[pos] = '\0';
                        if(pos) { furi_string_cat_str(app->log, line); furi_string_cat_str(app->log, "\n"); }
                        pos = 0;
                    } else line[pos++] = b;
                }
            }
            ...
            if(furi_string_size(app->log) < 30)
                furi_string_cat_str(app->log, "(no response)\n");
        }
        text_box_set_text(app->log_box, furi_string_get_cstr(app->log));
    }
}
